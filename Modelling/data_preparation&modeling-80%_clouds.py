...
def normalize_input(optical_images):
    ...
    try:
        ...
        for j in range(num_bands):
            normalized_band = normalize_band(
                optical_images[j]
            )  # Access each band directly
...
def process_files_in_folder(planet_file_paths, chm_file_paths):
    ...
    for planet_file_path, chm_file_path in zip(planet_file_paths, chm_file_paths):
        try:
            with xr.open_dataset(planet_file_path) as planet_data, xr.open_dataset(
                chm_file_path
            ) as chm_data:
                ...
                try:
                    ...
                    new_filename = (
                        os.path.splitext(planet_file_path)[0] + "_reprojected.tif"
                    )
                    reprojected_planet.band_data.astype(np.uint8).rio.to_raster(
                        new_filename
                    )
                    print(
                        f"Data type conversion to np.uint8 and saving successful for {new_filename}."
                    )
                    reprojected_files.append(
                        new_filename
                    )  # Add the reprojected file path to the list
                    ...
                    try:
                        ...
                        reprojected_planet.band_data.astype(np.float32).rio.to_raster(
                            new_filename
                        )
                        print(
                            f"Data type conversion to np.float32 and saving successful for {new_filename}."
                        )
                        reprojected_files.append(
                            new_filename
                        )  # Add the reprojected file path to the list
                        ...
                        print(
                            f"Data type conversion to np.uint8 and np.float32 failed for {planet_file_path}."
                        )
            ...
            print(
                f"Error processing files {planet_file_path} and {chm_file_path}: {str(e)}"
            )
...
def process_and_save_input_patches(
    input_folder, patch_size, valid_pixel_threshold=80.0, overlap=0
):
    """
    ...
    for input_file in input_files:
        ...
        with rasterio.open(input_path) as input_ds:
            ...
            windows_list = [
                w for w in generate_data_window(input_ds, patch_size, overlap)
            ]
            ...
            def process(w):
                patch = input_image[
                    :,
                    w.row_off : w.row_off + patch_size,
                    w.col_off : w.col_off + patch_size,
                ]
...
def process_and_save_target_patches(target_folder, patch_size, overlap=0):
    ...
    for target_file in target_files:
        ...
        with rasterio.open(target_path) as target_ds:
            ...
            windows_list = [
                w for w in generate_data_window(target_ds, patch_size, overlap)
            ]
...
def find_matching_pairs(input_folder, target_folder):
    ...
    """
    input_files = sorted(
        glob.glob(os.path.join(input_folder, "input_patches", "*.npy"))
    )
    target_files = sorted(
        glob.glob(os.path.join(target_folder, "target_patches", "*.npy"))
    )
    ...
    for target_file in target_files:
        ...
        target_pattern = re.findall(
            r"Polygon_(\d+_utm_\d+\w+)_patch_(\d+_\d+)", target_file
        )
        ...
        for input_file in input_files:
            ...
            input_pattern = re.findall(
                r"Polygon_(\d+_utm_\d+\w+)_merged_modified_patch_(\d+_\d+)", input_file
            )
    ...
    for unmatched_input_file in unmatched_input_files:
        try:
            ...
            print(
                f"Error removing unmatched input file {os.path.basename(unmatched_input_file)}: {str(e)}"
            )
    ...
    for unmatched_target_file in unmatched_target_files:
        try:
            ...
            print(
                f"Error removing unmatched target file {os.path.basename(unmatched_target_file)}: {str(e)}"
            )
...
def load_and_preprocess_data(planet_folder_path, lidar_folder_path, output_folder):
    ...
    #! I manually removed the following outliers: 222, 94, 97, 62, 66, 65, 4.
    ...
    chm_file_paths = [
        os.path.join(lidar_folder_path, chm_file) for _, chm_file in matching_pairs
    ]
    planet_file_names = [
        os.path.basename(planet_file) for planet_file, _ in matching_pairs
    ]
    ...
    for target_file_path in chm_file_paths:
        ...
        target_patch_names.append(
            os.path.basename(target_file_path)
        )  # Store the target patch names

    # Filter the matching pairs for the target patches using the stored names
    matching_pairs_target = [
        (planet_file, chm_file)
        for planet_file, chm_file in matching_pairs
        if os.path.basename(chm_file) in target_patch_names
    ]

    # Load, normalize, and store input patches
    normalized_input_patches = []
    for planet_file, _ in matching_pairs_target:
        input_file_path = os.path.join(planet_folder_path, planet_file)
    ...
    # Set a smaller batch size for splitting
    batch_size = 1000  # You can adjust this based on your memory capacity

    # Create memory-mapped files for the input and target arrays
    input_mmap = np.memmap(
        "input_memory_mapped.dat",
        dtype="float32",
        mode="w+",
        shape=rounded_input_patches_array.shape,
    )
    target_mmap = np.memmap(
        "target_memory_mapped.dat",
        dtype="float32",
        mode="w+",
        shape=rounded_target_patches_array.shape,
    )

    # Copy data to the memory-mapped arrays
    input_mmap[:] = rounded_input_patches_array[:]
    target_mmap[:] = rounded_target_patches_array[:]
    ...
    for i in range(0, len(input_mmap), batch_size):
        batch_input = input_mmap[i : i + batch_size]
        batch_target = target_mmap[i : i + batch_size]
        ...
        (
            ...
            train_target_batch,
            test_target_batch,
        ) = train_test_split(batch_input, batch_target, test_size=0.2)

        train_input.append(train_input_batch)
        test_input.append(test_input_batch)
        train_target.append(train_target_batch)
        test_target.append(test_target_batch)

    # Concatenate memory-mapped arrays for train and test sets
...
planet_folder_path = r"C:\Users\mpetel\Documents\Kalimatan Project\Code\Data\Output\planet_tiles\Processed Planet\input_patches"
lidar_folder_path = r"C:\Users\mpetel\Documents\Kalimatan Project\Code\Data\Output\LiDAR\Processed LiDAR\target_patches"
output_folder = r"C:\Users\mpetel\Documents\Kalimatan Project\Code\Data\Output\First Model - July 23 - U-Net - 5000 epochs\Within 80%"
...
# # Path to .npy file
# npy_file_path = r"C:\Users\mpetel\Documents\Kalimatan Project\Code\Data\Output\planet_tiles\Processed Planet\input_patches\Polygon_001_utm_50S_merged_modified_patch_768_4096.npy"
...
# # Path to .npy files
# #folder_path = r'C:\Users\mpetel\Documents\Kalimatan Project\Code\Data\Output\LiDAR\Processed LiDAR\target_patches'
# folder_path = r'C:\Users\mpetel\Documents\Kalimatan Project\Code\Data\Output\planet_tiles\Processed Planet\input_patches'
...
# # Extract the common prefix from the input file name
# input_file_prefix = random_input_file.split('_merged_modified')[0]

# # Find the corresponding target npy file
# matching_target_files = [
#     file for file in target_npy_files if input_file_prefix in file
# ]
...
# ----------------------------------------------------------------------------------------------------------------------------
# Checking the correspndance of the data
# ---------------------------------------------------------------------------------------------------------------------------

# def visualize_input_target_sets(train_input, train_target, test_input, test_target, val_input, val_target, indices=None):
#     """
#     Visualize input and target images from specified indices for different sets.

#     :param train_input: Array containing training input images
#     :param train_target: Array containing training target images
#     :param test_input: Array containing test input images
#     :param test_target: Array containing test target images
#     :param val_input: Array containing validation input images
#     :param val_target: Array containing validation target images
#     :param indices: List of indices to visualize (if None, random indices will be used)
#     """
#     num_examples = len(indices) if indices else 3

#     if not indices:
#         # Randomly select num_examples indices
#         indices = np.random.choice(train_input.shape[0], num_examples, replace=False)

#     plt.figure(figsize=(15, 18))

#     for i, idx in enumerate(indices):
#         train_input_img = train_input[idx].permute(1, 2, 0).cpu().numpy()
#         train_target_img = train_target[idx].unsqueeze(0).unsqueeze(3).cpu().numpy()
#         test_input_img = test_input[idx].permute(1, 2, 0).cpu().numpy()
#         test_target_img = test_target[idx].unsqueeze(0).unsqueeze(3).cpu().numpy()
#         val_input_img = val_input[idx].permute(1, 2, 0).cpu().numpy()
#         val_target_img = val_target[idx].unsqueeze(0).unsqueeze(3).cpu().numpy()

#         plt.subplot(num_examples, 6, i * 6 + 1)
#         plt.imshow(train_input_img[:, :, 3], vmin=0.000, vmax=0.8)
#         plt.title(f"Train Input {idx}")
#         plt.axis("off")

#         plt.subplot(num_examples, 6, i * 6 + 2)
#         plt.imshow(train_target_img.squeeze(), cmap="coolwarm")
#         plt.title(f"Train Target {idx}")
#         plt.axis("off")

#         plt.subplot(num_examples, 6, i * 6 + 3)
#         plt.imshow(test_input_img[:, :, 3], vmin=0.000, vmax=0.8)
#         plt.title(f"Test Input {idx}")
#         plt.axis("off")

#         plt.subplot(num_examples, 6, i * 6 + 4)
#         plt.imshow(test_target_img.squeeze(), cmap="coolwarm")
#         plt.title(f"Test Target {idx}")
#         plt.axis("off")

#         plt.subplot(num_examples, 6, i * 6 + 5)
#         plt.imshow(val_input_img[:, :, 3], vmin=0.000, vmax=0.8)
#         plt.title(f"Validation Input {idx}")
#         plt.axis("off")

#         plt.subplot(num_examples, 6, i * 6 + 6)
#         plt.imshow(val_target_img.squeeze(), cmap="coolwarm")
#         plt.title(f"Validation Target {idx}")
#         plt.axis("off")
...
# indices_to_visualize = [1970, 270, 200]

# # Call the function with specified indices
# visualize_input_target_sets(train_input_patches, train_target_patches, test_input_patches, test_target_patches, val_input_patches, val_target_patches, indices=indices_to_visualize)
...
class RandomColorJitter:
    ...
    def __call__(self, img):
        ...
        img = img_mean + img_saturation * (
            1 + torch.rand_like(img_saturation) * self.saturation
        )
        # Apply hue
        num_channels = img.size(0)
        for i in range(
            min(3, num_channels)
        ):  # Apply hue to the first 3 channels (excluding the 4th channel if present)
            img[i] += self.hue
        return img
...
transform = transforms.Compose(
    [
        ...
        ),  # Use the custom RandomColorJitter
        transforms.GaussianBlur(5, sigma=(0.1, 2.0)),
        RandomAdjustSharpness(
            sharpness_range=(0.0, 1.0)
        ),  # Use the custom RandomAdjustSharpness
...
def apply_median_filter(images, size):
    ...
    return torch.stack(
        [
            transforms.median_filter(image.unsqueeze(0), kernel_size=size)
            for image in images
        ]
    )
...
train_input_patches_patch = r"C:\Users\mpetel\Documents\Kalimatan Project\Code\Data\Output\First Model - July 23 - U-Net - 5000 epochs\Within 80%\train_input_patches.pth"
...
):  # Should not be neede, to check if results make sense
    """
...
(
    ...
    val_input_patches, val_target_patches, test_input_patches, test_target_patches
)
